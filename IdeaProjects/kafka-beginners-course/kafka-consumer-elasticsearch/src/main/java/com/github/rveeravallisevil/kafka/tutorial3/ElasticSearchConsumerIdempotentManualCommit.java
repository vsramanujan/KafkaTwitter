...
public class ElasticSearchConsumerIdempotentManualCommit {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Logger logger = LoggerFactory.getLogger(ElasticSearchConsumerIdempotentManualCommit.class.getName());
...
//        logger.info(id);
        ...
        while (true) {
            ...
            logger.info("Received " + records.count() + "count");
            for (ConsumerRecord<String, String> record : records) { //Reads from beginning stuff partition by partition but new messages are read as they arrive
               ...
               // String id = record.topic() + "_" + record.partition() + "_" + record.offset(); // Guaranteed to be unique
                ...
                IndexResponse indexResponse = client.index(indexRequest, RequestOptions.DEFAULT);
                //String id = indexResponse.getId(); // To make it idempotent we need to tell elastic search to use an id instead of getting the ID in return
