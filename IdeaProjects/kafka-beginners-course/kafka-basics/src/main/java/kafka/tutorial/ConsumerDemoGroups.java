...
public class ConsumerDemoGroups { // When you run this there are some logs that get printed in which you can see ' Setting newly assigned partitions [first_topic-0, first_topic-1, first_topic-2] ' which partition is assigned to his consumer instance -> run multiple
   //Whenever you spawn a new consumer this log comes where in re-balancing happens - these logs are printed by the Consumer Co-ordinator
